...
SELECT m.dept_no, m.dept_name, e.emp_no, e.last_name, e.first_name
FROM employees e
JOIN dept_manager d ON e.emp_no = d.emp_no
JOIN departments m ON m.dept_no = d.dept_no;

-- List the department number for each employee along with that employee’s 
-- employee number, last name, first name, and department name.

SELECT f.dept_no, e.emp_no, e.last_name, e.first_name, d.dept_name
FROM employees e 
JOIN dept_emp f ON e.emp_no = f.emp_no
JOIN departments d ON d.dept_no = f.dept_no;
